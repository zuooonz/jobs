...
async function fetchJobsOnePage(page, keyword, pageNum) {
  const encodedKeyword = encodeURIComponent(keyword);
  const url = `https://www.liepin.com/zhaopin/?key=${encodedKeyword}&city=010&dq=010&currentPage=${pageNum}`;
...
async function runOneKeywordAndSaveToDB(page, dbClient, keyword) {
  ...
  while (pageNum <= maxPageFound) {
    ...
    // 清洗和过滤: 去参数，保留所有抓到的岗位用于纯粹的数据库信息补全
    ...
    if (validJobs.length === 0) {
      ...
      for (const job of validJobs) {
        try {
          // 纯粹的修补逻辑：只基于 link 更新地点、公司和薪资，不执行无中生有的 INSERT
          const query = `
      ...
      console.log(`[${keyword}] 第${pageNum}页成功基于URL补全了库里 ${pageSaved} 个已知职位信息 (忽略了 ${validJobs.length - pageSaved} 个新职位)`);
    }
...
async function main() {
  ...
  for (let i = 0; i < picked.length; i++) {
    const kw = picked[i];
    const result = await runOneKeywordAndSaveToDB(page, dbClient, kw);
  ...
  console.log(`\n================ Session汇总 ================`);
  summaries.forEach(s => {
    console.log(`- ${s.keyword}: 成功基于URL补全了库里 ${s.totalSaved} 个已知职位信息`);
  });
